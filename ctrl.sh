...
#        ./crntb-adm.sh uninstall
        echo "Stopping league.7x24hs.com service, begin killing ${PID}"
    ...
    if [ "$PID" != "" ] ; then
        echo "Starting league.7x24hs.com service..."
        nohup python index.py -log_file_prefix=./log/league.log &
        echo $! > pidfile    #record process id to file
        echo 'Startup league.7x24hs.com service success!'
